...
def format_ip_label(ip):
    # Add spacing and optional line break for readability
    if len(ip) > 15:
        return ip[:12] + "\n" + ip[12:]  # split long IPv6 / MAC
    return ip
...
def build_sankey(df: pd.DataFrame, top_n: int, metric: str):
    ...
    fig.update_layout(
        font=dict(
            size=16,  # increase size
            family="Courier New, monospace",  # clearer for IPs
            color="black"
        )
    )
    fig.update_traces(
